...
def _db():
    db_path = getattr(current_app, "DB_PATH", None) or Path("/var/www/site/data/picture.db")
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    return conn

@app.route("/jid/visions", methods=["GET"])
def list_visions():
    """List visions with counts (supports ?search, ?limit, ?offset)."""
    search = (request.args.get("search") or "").strip()
    limit = int(request.args.get("limit") or 25)
    offset = int(request.args.get("offset") or 0)
    q = """
      SELECT v.id, v.vision AS vision_text, v.created_at,
             COALESCE(p.pic_count, 0) AS picture_count,
             COALESCE(f.focus_count, 0) AS focus_count
      FROM visions v
      LEFT JOIN (
          SELECT vision_id, COUNT(*) AS pic_count
          FROM pictures GROUP BY vision_id
      ) p ON p.vision_id = v.id
      LEFT JOIN (
          SELECT vision_id, COUNT(*) AS focus_count
          FROM focuses GROUP BY vision_id
      ) f ON f.vision_id = v.id
      WHERE (? = '' OR v.vision LIKE '%' || ? || '%')
      ORDER BY v.created_at DESC, v.id DESC
      LIMIT ? OFFSET ?
    """
    with _db() as cx:
        rows = cx.execute(q, (search, search, limit, offset)).fetchall()
    return jsonify({"items":[dict(r) for r in rows], "limit":limit, "offset":offset})

@app.route("/jid/visions/<int:vision_id>", methods=["GET"])
def get_vision(vision_id):
    with _db() as cx:
        v = cx.execute("SELECT id, vision AS vision_text, created_at FROM visions WHERE id=?",
                       (vision_id,)).fetchone()
        if not v:
            return jsonify({"error":"vision not found"}), 404
        pics = cx.execute("""
            SELECT id, vision_id, focus, title, description, function, explanation, created_at
            FROM pictures WHERE vision_id=? ORDER BY created_at DESC, id DESC
        """,(vision_id,)).fetchall()
        focs = cx.execute("""
            SELECT id, vision_id, focus AS focus_text, created_at
            FROM focuses WHERE vision_id=? ORDER BY created_at DESC, id DESC
        """,(vision_id,)).fetchall()
    return jsonify({
        "vision": dict(v),
        "pictures": [dict(r) for r in pics],
        "focuses": [dict(r) for r in focs],
    })

@app.route("/jid/visions/<int:vision_id>/pictures", methods=["GET"])
def list_pictures_for_vision(vision_id):
    """Optional filter by ?focus contains text."""
    focus_like = (request.args.get("focus") or "").strip()
    q = """
      SELECT id, vision_id, focus, title, description, function, explanation, created_at
      FROM pictures
      WHERE vision_id=? AND (?='' OR COALESCE(focus,'') LIKE '%' || ? || '%')
      ORDER BY created_at DESC, id DESC
    """
    with _db() as cx:
        rows = cx.execute(q, (vision_id, focus_like, focus_like)).fetchall()
    return jsonify({"items":[dict(r) for r in rows]})

@app.route("/jid/visions/<int:vision_id>/focuses", methods=["GET"])
def list_focuses_for_vision(vision_id):
    with _db() as cx:
        rows = cx.execute("""
          SELECT id, vision_id, focus AS focus_text, created_at
          FROM focuses WHERE vision_id=? ORDER BY created_at DESC, id DESC
...
@app.route("/jid/pictures/<int:picture_id>", methods=["GET"])
def get_picture(picture_id):
    with _db() as cx:
        ...
        v = cx.execute("SELECT id, vision AS vision_text FROM visions WHERE id=?",
                       (p["vision_id"],)).fetchone()
    out = dict(p)
    out["vision"] = dict(v) if v else None
    return jsonify(out)
